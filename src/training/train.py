...
def encode_target_col(
    ...
    with open(
        os.path.join(MODEL_PATH, f"{model_1_name}-{model_2_name}", "model_target_translator.pkl"), "wb"
    ) as pkl:
...
def trainer(X, y, model_name: str, logger) -> None:
    ...
    logger.info(f"Training final models with best found parameters")
    ...
    logger.info(f"Both models (XGB & CatBoost) trained and saved successfully in {save_path}")
